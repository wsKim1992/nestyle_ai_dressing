...
from VITON.viton_exp import test_for_test
...
def dress_on2(model_src, dress_src):
    ...
    dress_name = '.'.join(dress_src.split('/')[-1].split('.')[:-1])
    ...
    filename = f"{model_name}_{dress_name}"
    
    err = test_for_test(opt, inputs, segnet, clothflow, generator, name=filename, save_dir='./static/img_fitting')  # from VITON.segmentation
    if err:
...
if __name__ == '__main__' :
    ...
    for model in range(1, 11):
        for cloth in range(1, 12):
            dress_on2(f"static/img-sample-new/model/model{model:0>2}.jpg",
                      f"static/img-sample-new/cloth/cloth{cloth:0>2}.jpg")
